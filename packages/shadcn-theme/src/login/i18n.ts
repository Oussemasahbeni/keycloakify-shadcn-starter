...
/**
 * Translations for locales outside @keycloakify/login-ui's 30-language default set.
 * The builder's type only admits the default set, but at runtime custom translations
 * are looked up by the realm's actual language tag, so these are still served when a
 * realm enables one of these locales. (Base Keycloak messages for them fall back to
 * English unless the locale is registered via i18nBuilder.withExtraLanguages.)
 */
const translations_notInDefaultSet = {
    az: {
...
const { I18nProvider, useI18n } = i18nBuilder
    ...
    .withCustomTranslations({
        ...(translations_notInDefaultSet as Record<never, never>),
        en: {
